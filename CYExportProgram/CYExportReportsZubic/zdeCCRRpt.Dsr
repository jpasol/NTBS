VERSION 5.00
Begin {C0E45035-5775-11D0-B388-00A0C9055D8E} zdeCCRRpt 
   ClientHeight    =   14040
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   22155
   _ExtentX        =   39079
   _ExtentY        =   24765
   FolderFlags     =   1
   TypeLibGuid     =   "{3EBBC2C0-06E6-11D3-9F1C-00105A626E67}"
   TypeInfoGuid    =   "{3EBBC2C1-06E6-11D3-9F1C-00105A626E67}"
   TypeInfoCookie  =   0
   Version         =   4
   NumConnections  =   1
   BeginProperty Connection1 
      ConnectionName  =   "Billing"
      ConnDispId      =   1001
      SourceOfData    =   3
      ConnectionSource=   "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=billing;Data Source=SBITCBILLING"
      Expanded        =   -1  'True
      IsSQL           =   -1  'True
      QuoteChar       =   34
      SeparatorChar   =   46
   EndProperty
   NumRecordsets   =   20
   BeginProperty Recordset1 
      CommandName     =   "getInformation"
      CommDispId      =   1003
      RsDispId        =   1008
      CommandText     =   "select username = user_name(),workstation=host_name()"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   2
      BeginProperty Field1 
         Precision       =   0
         Size            =   128
         Scale           =   0
         Type            =   202
         Name            =   "username"
         Caption         =   "username"
      EndProperty
      BeginProperty Field2 
         Precision       =   0
         Size            =   128
         Scale           =   0
         Type            =   202
         Name            =   "workstation"
         Caption         =   "workstation"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset2 
      CommandName     =   "getConsLiquidator"
      CommDispId      =   1009
      RsDispId        =   1030
      CommandText     =   "SELECT * FROM ccrcyx WHERE sysdttm > ? AND sysdttm < ? "
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   39
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   3
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "seqnum"
         Caption         =   "seqnum"
      EndProperty
      BeginProperty Field3 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   129
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field4 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ccrnum"
         Caption         =   "ccrnum"
      EndProperty
      BeginProperty Field5 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "fulemp"
         Caption         =   "fulemp"
      EndProperty
      BeginProperty Field7 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "dgrcls"
         Caption         =   "dgrcls"
      EndProperty
      BeginProperty Field8 
         Precision       =   0
         Size            =   7
         Scale           =   0
         Type            =   129
         Name            =   "vslcde"
         Caption         =   "vslcde"
      EndProperty
      BeginProperty Field9 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field10 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arramt"
         Caption         =   "arramt"
      EndProperty
      BeginProperty Field11 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ovzamt"
         Caption         =   "ovzamt"
      EndProperty
      BeginProperty Field12 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dgramt"
         Caption         =   "dgramt"
      EndProperty
      BeginProperty Field13 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field14 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      BeginProperty Field15 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "vatcde"
         Caption         =   "vatcde"
      EndProperty
      BeginProperty Field16 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzl"
         Caption         =   "cntovzl"
      EndProperty
      BeginProperty Field17 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzw"
         Caption         =   "cntovzw"
      EndProperty
      BeginProperty Field18 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzh"
         Caption         =   "cntovzh"
      EndProperty
      BeginProperty Field19 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ovzums"
         Caption         =   "ovzums"
      EndProperty
      BeginProperty Field20 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "revton"
         Caption         =   "revton"
      EndProperty
      BeginProperty Field21 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "trncde"
         Caption         =   "trncde"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "whfcde"
         Caption         =   "whfcde"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "guarntycde"
         Caption         =   "guarntycde"
      EndProperty
      BeginProperty Field24 
         Precision       =   5
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dolrte"
         Caption         =   "dolrte"
      EndProperty
      BeginProperty Field25 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "exprtr"
         Caption         =   "exprtr"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "broker"
         Caption         =   "broker"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   80
         Scale           =   0
         Type            =   200
         Name            =   "entnum"
         Caption         =   "entnum"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "commod"
         Caption         =   "commod"
      EndProperty
      BeginProperty Field29 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "remark"
         Caption         =   "remark"
      EndProperty
      BeginProperty Field30 
         Precision       =   0
         Size            =   20
         Scale           =   0
         Type            =   200
         Name            =   "trknam"
         Caption         =   "trknam"
      EndProperty
      BeginProperty Field31 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "pltnum"
         Caption         =   "pltnum"
      EndProperty
      BeginProperty Field32 
         Precision       =   0
         Size            =   35
         Scale           =   0
         Type            =   200
         Name            =   "trkchs"
         Caption         =   "trkchs"
      EndProperty
      BeginProperty Field33 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field34 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ovrccr"
         Caption         =   "ovrccr"
      EndProperty
      BeginProperty Field35 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ppanum"
         Caption         =   "ppanum"
      EndProperty
      BeginProperty Field36 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field37 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field38 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field39 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "outdttm"
         Caption         =   "outdttm"
      EndProperty
      NumGroups       =   0
      ParamCount      =   2
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset3 
      CommandName     =   "getExemption"
      CommDispId      =   1014
      RsDispId        =   1070
      CommandText     =   "SELECT * FROM CCRcyx WHERE whfcde <> '0' AND status <> 'CAN' AND sysdttm >= ? AND sysdttm <= ?"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   40
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   3
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "seqnum"
         Caption         =   "seqnum"
      EndProperty
      BeginProperty Field3 
         Precision       =   1
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "itmnum"
         Caption         =   "itmnum"
      EndProperty
      BeginProperty Field4 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   129
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field5 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ccrnum"
         Caption         =   "ccrnum"
      EndProperty
      BeginProperty Field6 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field7 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "fulemp"
         Caption         =   "fulemp"
      EndProperty
      BeginProperty Field8 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "dgrcls"
         Caption         =   "dgrcls"
      EndProperty
      BeginProperty Field9 
         Precision       =   0
         Size            =   7
         Scale           =   0
         Type            =   129
         Name            =   "vslcde"
         Caption         =   "vslcde"
      EndProperty
      BeginProperty Field10 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field11 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arramt"
         Caption         =   "arramt"
      EndProperty
      BeginProperty Field12 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ovzamt"
         Caption         =   "ovzamt"
      EndProperty
      BeginProperty Field13 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dgramt"
         Caption         =   "dgramt"
      EndProperty
      BeginProperty Field14 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field15 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      BeginProperty Field16 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "vatcde"
         Caption         =   "vatcde"
      EndProperty
      BeginProperty Field17 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzl"
         Caption         =   "cntovzl"
      EndProperty
      BeginProperty Field18 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzw"
         Caption         =   "cntovzw"
      EndProperty
      BeginProperty Field19 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzh"
         Caption         =   "cntovzh"
      EndProperty
      BeginProperty Field20 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ovzums"
         Caption         =   "ovzums"
      EndProperty
      BeginProperty Field21 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "revton"
         Caption         =   "revton"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "trncde"
         Caption         =   "trncde"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "whfcde"
         Caption         =   "whfcde"
      EndProperty
      BeginProperty Field24 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "guarntycde"
         Caption         =   "guarntycde"
      EndProperty
      BeginProperty Field25 
         Precision       =   5
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dolrte"
         Caption         =   "dolrte"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "exprtr"
         Caption         =   "exprtr"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "broker"
         Caption         =   "broker"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   80
         Scale           =   0
         Type            =   200
         Name            =   "entnum"
         Caption         =   "entnum"
      EndProperty
      BeginProperty Field29 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "commod"
         Caption         =   "commod"
      EndProperty
      BeginProperty Field30 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "remark"
         Caption         =   "remark"
      EndProperty
      BeginProperty Field31 
         Precision       =   0
         Size            =   20
         Scale           =   0
         Type            =   200
         Name            =   "trknam"
         Caption         =   "trknam"
      EndProperty
      BeginProperty Field32 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "pltnum"
         Caption         =   "pltnum"
      EndProperty
      BeginProperty Field33 
         Precision       =   0
         Size            =   35
         Scale           =   0
         Type            =   200
         Name            =   "trkchs"
         Caption         =   "trkchs"
      EndProperty
      BeginProperty Field34 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field35 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ovrccr"
         Caption         =   "ovrccr"
      EndProperty
      BeginProperty Field36 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ppanum"
         Caption         =   "ppanum"
      EndProperty
      BeginProperty Field37 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field38 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field39 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field40 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "outdttm"
         Caption         =   "outdttm"
      EndProperty
      NumGroups       =   0
      ParamCount      =   2
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset4 
      CommandName     =   "GetUserType"
      CommDispId      =   1017
      RsDispId        =   1022
      CommandText     =   "select * from userinfo where userid = ?"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   8
      BeginProperty Field1 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field2 
         Precision       =   0
         Size            =   40
         Scale           =   0
         Type            =   200
         Name            =   "usrnam"
         Caption         =   "usrnam"
      EndProperty
      BeginProperty Field3 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "usrpos"
         Caption         =   "usrpos"
      EndProperty
      BeginProperty Field4 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "dptcde"
         Caption         =   "dptcde"
      EndProperty
      BeginProperty Field5 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "offcde"
         Caption         =   "offcde"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field7 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field8 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdte"
         Caption         =   "sysdte"
      EndProperty
      NumGroups       =   0
      ParamCount      =   1
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   255
         Scale           =   255
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset5 
      CommandName     =   "getTotalAdr"
      CommDispId      =   1023
      RsDispId        =   1029
      CommandText     =   $"zdeCCRRpt.dsx":0000
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   4
      BeginProperty Field1 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalAdr"
         Caption         =   "TotalAdr"
      EndProperty
      BeginProperty Field2 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalCsh"
         Caption         =   "TotalCsh"
      EndProperty
      BeginProperty Field3 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalChq"
         Caption         =   "TotalChq"
      EndProperty
      BeginProperty Field4 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "Total"
         Caption         =   "Total"
      EndProperty
      NumGroups       =   0
      ParamCount      =   2
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset6 
      CommandName     =   "getLiquidator"
      CommDispId      =   1031
      RsDispId        =   1128
      CommandText     =   "SELECT * FROM ccrcyx WHERE sysdttm >=? AND sysdttm <= ?  and userid = ?"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   41
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   3
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "seqnum"
         Caption         =   "seqnum"
      EndProperty
      BeginProperty Field3 
         Precision       =   1
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "itmnum"
         Caption         =   "itmnum"
      EndProperty
      BeginProperty Field4 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   129
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field5 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ccrnum"
         Caption         =   "ccrnum"
      EndProperty
      BeginProperty Field6 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field7 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "fulemp"
         Caption         =   "fulemp"
      EndProperty
      BeginProperty Field8 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "dgrcls"
         Caption         =   "dgrcls"
      EndProperty
      BeginProperty Field9 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "vslcde"
         Caption         =   "vslcde"
      EndProperty
      BeginProperty Field10 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field11 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arramt"
         Caption         =   "arramt"
      EndProperty
      BeginProperty Field12 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ovzamt"
         Caption         =   "ovzamt"
      EndProperty
      BeginProperty Field13 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dgramt"
         Caption         =   "dgramt"
      EndProperty
      BeginProperty Field14 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field15 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      BeginProperty Field16 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "vatcde"
         Caption         =   "vatcde"
      EndProperty
      BeginProperty Field17 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzl"
         Caption         =   "cntovzl"
      EndProperty
      BeginProperty Field18 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzw"
         Caption         =   "cntovzw"
      EndProperty
      BeginProperty Field19 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzh"
         Caption         =   "cntovzh"
      EndProperty
      BeginProperty Field20 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ovzums"
         Caption         =   "ovzums"
      EndProperty
      BeginProperty Field21 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "revton"
         Caption         =   "revton"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "trncde"
         Caption         =   "trncde"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "whfcde"
         Caption         =   "whfcde"
      EndProperty
      BeginProperty Field24 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "guarntycde"
         Caption         =   "guarntycde"
      EndProperty
      BeginProperty Field25 
         Precision       =   5
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dolrte"
         Caption         =   "dolrte"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "exprtr"
         Caption         =   "exprtr"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "broker"
         Caption         =   "broker"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   80
         Scale           =   0
         Type            =   200
         Name            =   "entnum"
         Caption         =   "entnum"
      EndProperty
      BeginProperty Field29 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "commod"
         Caption         =   "commod"
      EndProperty
      BeginProperty Field30 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "remark"
         Caption         =   "remark"
      EndProperty
      BeginProperty Field31 
         Precision       =   0
         Size            =   20
         Scale           =   0
         Type            =   200
         Name            =   "trknam"
         Caption         =   "trknam"
      EndProperty
      BeginProperty Field32 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "pltnum"
         Caption         =   "pltnum"
      EndProperty
      BeginProperty Field33 
         Precision       =   0
         Size            =   35
         Scale           =   0
         Type            =   200
         Name            =   "trkchs"
         Caption         =   "trkchs"
      EndProperty
      BeginProperty Field34 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field35 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ovrccr"
         Caption         =   "ovrccr"
      EndProperty
      BeginProperty Field36 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ppanum"
         Caption         =   "ppanum"
      EndProperty
      BeginProperty Field37 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field38 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field39 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field40 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "outdttm"
         Caption         =   "outdttm"
      EndProperty
      BeginProperty Field41 
         Precision       =   0
         Size            =   20
         Scale           =   0
         Type            =   129
         Name            =   "supvsr"
         Caption         =   "supvsr"
      EndProperty
      NumGroups       =   0
      ParamCount      =   3
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "Param3"
         Direction       =   1
         Precision       =   255
         Scale           =   255
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset7 
      CommandName     =   "CCRCyx"
      CommDispId      =   1037
      RsDispId        =   1063
      CommandText     =   $"zdeCCRRpt.dsx":011A
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   44
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   3
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "seqnum"
         Caption         =   "seqnum"
      EndProperty
      BeginProperty Field3 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   129
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field4 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ccrnum"
         Caption         =   "ccrnum"
      EndProperty
      BeginProperty Field5 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "fulemp"
         Caption         =   "fulemp"
      EndProperty
      BeginProperty Field7 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "dgrcls"
         Caption         =   "dgrcls"
      EndProperty
      BeginProperty Field8 
         Precision       =   0
         Size            =   7
         Scale           =   0
         Type            =   129
         Name            =   "vslcde"
         Caption         =   "vslcde"
      EndProperty
      BeginProperty Field9 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field10 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arramt"
         Caption         =   "arramt"
      EndProperty
      BeginProperty Field11 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ovzamt"
         Caption         =   "ovzamt"
      EndProperty
      BeginProperty Field12 
         Precision       =   8
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dgramt"
         Caption         =   "dgramt"
      EndProperty
      BeginProperty Field13 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field14 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      BeginProperty Field15 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "vatcde"
         Caption         =   "vatcde"
      EndProperty
      BeginProperty Field16 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzl"
         Caption         =   "cntovzl"
      EndProperty
      BeginProperty Field17 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzw"
         Caption         =   "cntovzw"
      EndProperty
      BeginProperty Field18 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cntovzh"
         Caption         =   "cntovzh"
      EndProperty
      BeginProperty Field19 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ovzums"
         Caption         =   "ovzums"
      EndProperty
      BeginProperty Field20 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "revton"
         Caption         =   "revton"
      EndProperty
      BeginProperty Field21 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "trncde"
         Caption         =   "trncde"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "whfcde"
         Caption         =   "whfcde"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "guarntycde"
         Caption         =   "guarntycde"
      EndProperty
      BeginProperty Field24 
         Precision       =   5
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "dolrte"
         Caption         =   "dolrte"
      EndProperty
      BeginProperty Field25 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "exprtr"
         Caption         =   "exprtr"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "broker"
         Caption         =   "broker"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   80
         Scale           =   0
         Type            =   200
         Name            =   "entnum"
         Caption         =   "entnum"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "commod"
         Caption         =   "commod"
      EndProperty
      BeginProperty Field29 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "remark"
         Caption         =   "remark"
      EndProperty
      BeginProperty Field30 
         Precision       =   0
         Size            =   20
         Scale           =   0
         Type            =   200
         Name            =   "trknam"
         Caption         =   "trknam"
      EndProperty
      BeginProperty Field31 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "pltnum"
         Caption         =   "pltnum"
      EndProperty
      BeginProperty Field32 
         Precision       =   0
         Size            =   35
         Scale           =   0
         Type            =   200
         Name            =   "trkchs"
         Caption         =   "trkchs"
      EndProperty
      BeginProperty Field33 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field34 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ovrccr"
         Caption         =   "ovrccr"
      EndProperty
      BeginProperty Field35 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "ppanum"
         Caption         =   "ppanum"
      EndProperty
      BeginProperty Field36 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field37 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field38 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field39 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "outdttm"
         Caption         =   "outdttm"
      EndProperty
      BeginProperty Field40 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Year"
         Caption         =   "Year"
      EndProperty
      BeginProperty Field41 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Day"
         Caption         =   "Day"
      EndProperty
      BeginProperty Field42 
         Precision       =   12
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "Arrastre"
         Caption         =   "Arrastre"
      EndProperty
      BeginProperty Field43 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "NetOfVat"
         Caption         =   "NetOfVat"
      EndProperty
      BeginProperty Field44 
         Precision       =   11
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "GrandTotal"
         Caption         =   "GrandTotal"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset8 
      CommandName     =   "CCRCyx2"
      CommDispId      =   1045
      RsDispId        =   1050
      CommandText     =   $"zdeCCRRpt.dsx":020B
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   10
      BeginProperty Field1 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Year"
         Caption         =   "Year"
      EndProperty
      BeginProperty Field2 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Day"
         Caption         =   "Day"
      EndProperty
      BeginProperty Field3 
         Precision       =   12
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "Arrastre"
         Caption         =   "Arrastre"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "NetOfVat"
         Caption         =   "NetOfVat"
      EndProperty
      BeginProperty Field5 
         Precision       =   11
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "GrandTotal"
         Caption         =   "GrandTotal"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   200
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field7 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field8 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field9 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field10 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset9 
      CommandName     =   "CCRCy"
      CommDispId      =   1051
      RsDispId        =   1062
      CommandText     =   $"zdeCCRRpt.dsx":03BE
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   10
      BeginProperty Field1 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "Year"
         Caption         =   "Year"
      EndProperty
      BeginProperty Field2 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "Day"
         Caption         =   "Day"
      EndProperty
      BeginProperty Field3 
         Precision       =   12
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "Arrastre"
         Caption         =   "Arrastre"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "NetOfVat"
         Caption         =   "NetOfVat"
      EndProperty
      BeginProperty Field5 
         Precision       =   11
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "GrandTotal"
         Caption         =   "GrandTotal"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   12
         Scale           =   0
         Type            =   129
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field7 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field8 
         Precision       =   6
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field9 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field10 
         Precision       =   8
         Size            =   19
         Scale           =   3
         Type            =   131
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset10 
      CommandName     =   "CCRDay"
      CommDispId      =   1057
      RsDispId        =   1060
      CommandText     =   $"zdeCCRRpt.dsx":0503
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   10
      BeginProperty Field1 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Year"
         Caption         =   "Year"
      EndProperty
      BeginProperty Field2 
         Precision       =   2
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "Day"
         Caption         =   "Day"
      EndProperty
      BeginProperty Field3 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "Arrastre"
         Caption         =   "Arrastre"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "NetOfVat"
         Caption         =   "NetOfVat"
      EndProperty
      BeginProperty Field5 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "GrandTotal"
         Caption         =   "GrandTotal"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   0
         Scale           =   0
         Type            =   200
         Name            =   "cntnum"
         Caption         =   "cntnum"
      EndProperty
      BeginProperty Field7 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "cntsze"
         Caption         =   "cntsze"
      EndProperty
      BeginProperty Field8 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "whfamt"
         Caption         =   "whfamt"
      EndProperty
      BeginProperty Field9 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field10 
         Precision       =   10
         Size            =   4
         Scale           =   0
         Type            =   3
         Name            =   "arrtax"
         Caption         =   "arrtax"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset11 
      CommandName     =   "getTellerTotalAdr"
      CommDispId      =   1064
      RsDispId        =   1104
      CommandText     =   $"zdeCCRRpt.dsx":0592
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   4
      BeginProperty Field1 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalAdr"
         Caption         =   "TotalAdr"
      EndProperty
      BeginProperty Field2 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalCsh"
         Caption         =   "TotalCsh"
      EndProperty
      BeginProperty Field3 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalChq"
         Caption         =   "TotalChq"
      EndProperty
      BeginProperty Field4 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "Total"
         Caption         =   "Total"
      EndProperty
      NumGroups       =   0
      ParamCount      =   3
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "Param3"
         Direction       =   1
         Precision       =   255
         Scale           =   255
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset12 
      CommandName     =   "getArrCyx"
      CommDispId      =   1071
      RsDispId        =   -1
      CommandText     =   "dbo.up_getarrastrewvatcyx"
      ActiveConnectionName=   "Billing"
      CallSyntax      =   "{? = CALL dbo.up_getarrastrewvatcyx( ?, ?, ?, ?) }"
      NumFields       =   0
      NumGroups       =   0
      ParamCount      =   5
      BeginProperty P1 
         RealName        =   "RETURN_VALUE"
         Direction       =   4
         Precision       =   10
         Scale           =   0
         Size            =   0
         DataType        =   3
         HostType        =   3
         Required        =   0   'False
      EndProperty
      BeginProperty P2 
         RealName        =   "@@pUser"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "@@pFromDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P4 
         RealName        =   "@@pToDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P5 
         RealName        =   "@@pArrastre"
         Direction       =   3
         Precision       =   12
         Scale           =   2
         Size            =   0
         DataType        =   131
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset13 
      CommandName     =   "getArrNoVatCyx"
      CommDispId      =   1076
      RsDispId        =   -1
      CommandText     =   "dbo.up_getarrastrenovatcyx"
      ActiveConnectionName=   "Billing"
      CallSyntax      =   "{? = CALL dbo.up_getarrastrenovatcyx( ?, ?, ?, ?) }"
      NumFields       =   0
      NumGroups       =   0
      ParamCount      =   5
      BeginProperty P1 
         RealName        =   "RETURN_VALUE"
         Direction       =   4
         Precision       =   10
         Scale           =   0
         Size            =   0
         DataType        =   3
         HostType        =   3
         Required        =   0   'False
      EndProperty
      BeginProperty P2 
         RealName        =   "@@pUser"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "@@pFromDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P4 
         RealName        =   "@@pToDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P5 
         RealName        =   "@@pArrastreNoVat"
         Direction       =   3
         Precision       =   12
         Scale           =   2
         Size            =   0
         DataType        =   131
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset14 
      CommandName     =   "getArrTaxCyx"
      CommDispId      =   1085
      RsDispId        =   -1
      CommandText     =   "dbo.up_getarrastretaxcyx"
      ActiveConnectionName=   "Billing"
      CallSyntax      =   "{? = CALL dbo.up_getarrastretaxcyx( ?, ?, ?, ?) }"
      NumFields       =   0
      NumGroups       =   0
      ParamCount      =   5
      BeginProperty P1 
         RealName        =   "RETURN_VALUE"
         Direction       =   4
         Precision       =   10
         Scale           =   0
         Size            =   0
         DataType        =   3
         HostType        =   3
         Required        =   0   'False
      EndProperty
      BeginProperty P2 
         RealName        =   "@@pUser"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "@@pFromDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P4 
         RealName        =   "@@pToDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P5 
         RealName        =   "@@pArrastre"
         Direction       =   3
         Precision       =   12
         Scale           =   2
         Size            =   0
         DataType        =   131
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset15 
      CommandName     =   "getArrVatCyx"
      CommDispId      =   1090
      RsDispId        =   -1
      CommandText     =   "dbo.up_getarrastrevatcyx"
      ActiveConnectionName=   "Billing"
      CallSyntax      =   "{? = CALL dbo.up_getarrastrevatcyx( ?, ?, ?, ?) }"
      NumFields       =   0
      NumGroups       =   0
      ParamCount      =   5
      BeginProperty P1 
         RealName        =   "RETURN_VALUE"
         Direction       =   4
         Precision       =   10
         Scale           =   0
         Size            =   0
         DataType        =   3
         HostType        =   3
         Required        =   0   'False
      EndProperty
      BeginProperty P2 
         RealName        =   "@@pUser"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "@@pFromDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P4 
         RealName        =   "@@pToDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P5 
         RealName        =   "@@pArrastreVat"
         Direction       =   3
         Precision       =   12
         Scale           =   2
         Size            =   0
         DataType        =   131
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset16 
      CommandName     =   "getAdrPaidCyx"
      CommDispId      =   1095
      RsDispId        =   -1
      CommandText     =   "dbo.up_getadrpaidcyx"
      ActiveConnectionName=   "Billing"
      CallSyntax      =   "{? = CALL dbo.up_getadrpaidcyx( ?, ?, ?, ?) }"
      NumFields       =   0
      NumGroups       =   0
      ParamCount      =   5
      BeginProperty P1 
         RealName        =   "RETURN_VALUE"
         Direction       =   4
         Precision       =   10
         Scale           =   0
         Size            =   0
         DataType        =   3
         HostType        =   3
         Required        =   0   'False
      EndProperty
      BeginProperty P2 
         RealName        =   "@@pUser"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "@@pFromDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P4 
         RealName        =   "@@pToDate"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   0
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P5 
         RealName        =   "@@pAdrpaid"
         Direction       =   3
         Precision       =   12
         Scale           =   2
         Size            =   0
         DataType        =   131
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset17 
      CommandName     =   "CyxSummary"
      CommDispId      =   1097
      RsDispId        =   1102
      CommandText     =   "dbo.CYSummary"
      ActiveConnectionName=   "Billing"
      CommandType     =   2
      dbObjectType    =   1
      Locktype        =   3
      IsRSReturning   =   -1  'True
      NumFields       =   29
      BeginProperty Field1 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arramt"
         Caption         =   "arramt"
      EndProperty
      BeginProperty Field2 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arrvat"
         Caption         =   "arrvat"
      EndProperty
      BeginProperty Field3 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arrnov"
         Caption         =   "arrnov"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "arrwtx"
         Caption         =   "arrwtx"
      EndProperty
      BeginProperty Field5 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "stoamt"
         Caption         =   "stoamt"
      EndProperty
      BeginProperty Field6 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "stovat"
         Caption         =   "stovat"
      EndProperty
      BeginProperty Field7 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "stonov"
         Caption         =   "stonov"
      EndProperty
      BeginProperty Field8 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "stowtx"
         Caption         =   "stowtx"
      EndProperty
      BeginProperty Field9 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "wghamt"
         Caption         =   "wghamt"
      EndProperty
      BeginProperty Field10 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "wghvat"
         Caption         =   "wghvat"
      EndProperty
      BeginProperty Field11 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "wghnov"
         Caption         =   "wghnov"
      EndProperty
      BeginProperty Field12 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "wghwtx"
         Caption         =   "wghwtx"
      EndProperty
      BeginProperty Field13 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "rframt"
         Caption         =   "rframt"
      EndProperty
      BeginProperty Field14 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "rfrvat"
         Caption         =   "rfrvat"
      EndProperty
      BeginProperty Field15 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "rfrnov"
         Caption         =   "rfrnov"
      EndProperty
      BeginProperty Field16 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "rfrwtx"
         Caption         =   "rfrwtx"
      EndProperty
      BeginProperty Field17 
         Precision       =   0
         Size            =   30
         Scale           =   0
         Type            =   200
         Name            =   "adrnum"
         Caption         =   "adrnum"
      EndProperty
      BeginProperty Field18 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adramt"
         Caption         =   "adramt"
      EndProperty
      BeginProperty Field19 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adrarr"
         Caption         =   "adrarr"
      EndProperty
      BeginProperty Field20 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adrsto"
         Caption         =   "adrsto"
      EndProperty
      BeginProperty Field21 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adrwgh"
         Caption         =   "adrwgh"
      EndProperty
      BeginProperty Field22 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adrrfr"
         Caption         =   "adrrfr"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "impexp"
         Caption         =   "impexp"
      EndProperty
      BeginProperty Field24 
         Precision       =   0
         Size            =   2
         Scale           =   0
         Type            =   129
         Name            =   "daycde"
         Caption         =   "daycde"
      EndProperty
      BeginProperty Field25 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field27 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "strdte"
         Caption         =   "strdte"
      EndProperty
      BeginProperty Field28 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "enddte"
         Caption         =   "enddte"
      EndProperty
      BeginProperty Field29 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdate"
         Caption         =   "sysdate"
      EndProperty
      NumGroups       =   0
      ParamCount      =   0
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset18 
      CommandName     =   "getCash"
      CommDispId      =   1110
      RsDispId        =   1115
      CommandText     =   "Select * FROM ccrpay WHERE ccrtyp = '1' AND sysdttm >= ? AND sysdttm <= ? AND status <> 'CAN' AND userid = ?"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   28
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   0
         Size            =   6
         Scale           =   0
         Type            =   129
         Name            =   "cuscde"
         Caption         =   "cuscde"
      EndProperty
      BeginProperty Field3 
         Precision       =   0
         Size            =   40
         Scale           =   0
         Type            =   200
         Name            =   "cusnam"
         Caption         =   "cusnam"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cshamt"
         Caption         =   "cshamt"
      EndProperty
      BeginProperty Field5 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adramt"
         Caption         =   "adramt"
      EndProperty
      BeginProperty Field6 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "adrnum"
         Caption         =   "adrnum"
      EndProperty
      BeginProperty Field7 
         Precision       =   9
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chgamt"
         Caption         =   "chgamt"
      EndProperty
      BeginProperty Field8 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno1"
         Caption         =   "chkno1"
      EndProperty
      BeginProperty Field9 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno2"
         Caption         =   "chkno2"
      EndProperty
      BeginProperty Field10 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno3"
         Caption         =   "chkno3"
      EndProperty
      BeginProperty Field11 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno4"
         Caption         =   "chkno4"
      EndProperty
      BeginProperty Field12 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno5"
         Caption         =   "chkno5"
      EndProperty
      BeginProperty Field13 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt1"
         Caption         =   "chkamt1"
      EndProperty
      BeginProperty Field14 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt2"
         Caption         =   "chkamt2"
      EndProperty
      BeginProperty Field15 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt3"
         Caption         =   "chkamt3"
      EndProperty
      BeginProperty Field16 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt4"
         Caption         =   "chkamt4"
      EndProperty
      BeginProperty Field17 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt5"
         Caption         =   "chkamt5"
      EndProperty
      BeginProperty Field18 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk1"
         Caption         =   "chkbnk1"
      EndProperty
      BeginProperty Field19 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk2"
         Caption         =   "chkbnk2"
      EndProperty
      BeginProperty Field20 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk3"
         Caption         =   "chkbnk3"
      EndProperty
      BeginProperty Field21 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk4"
         Caption         =   "chkbnk4"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk5"
         Caption         =   "chkbnk5"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field24 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "rectag"
         Caption         =   "rectag"
      EndProperty
      BeginProperty Field25 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field26 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ccrtyp"
         Caption         =   "ccrtyp"
      EndProperty
      NumGroups       =   0
      ParamCount      =   3
      BeginProperty P1 
         RealName        =   "Param1"
         UserName        =   "FrmTime"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         UserName        =   "ToTime"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "Param3"
         UserName        =   "Userid"
         Direction       =   1
         Precision       =   0
         Scale           =   0
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset19 
      CommandName     =   "getTellerAmt"
      CommDispId      =   1116
      RsDispId        =   1121
      CommandText     =   $"zdeCCRRpt.dsx":06CF
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   3
      BeginProperty Field1 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalCsh"
         Caption         =   "TotalCsh"
      EndProperty
      BeginProperty Field2 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "TotalChq"
         Caption         =   "TotalChq"
      EndProperty
      BeginProperty Field3 
         Precision       =   38
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "Total"
         Caption         =   "Total"
      EndProperty
      NumGroups       =   0
      ParamCount      =   3
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P3 
         RealName        =   "Param3"
         Direction       =   1
         Precision       =   255
         Scale           =   255
         Size            =   10
         DataType        =   200
         HostType        =   8
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
   BeginProperty Recordset20 
      CommandName     =   "getCashAll"
      CommDispId      =   1122
      RsDispId        =   1127
      CommandText     =   "Select * FROM ccrpay WHERE ccrtyp = '1' AND sysdttm >= ? AND sysdttm <= ? AND status <> 'CAN'"
      ActiveConnectionName=   "Billing"
      CommandType     =   1
      IsRSReturning   =   -1  'True
      NumFields       =   31
      BeginProperty Field1 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "refnum"
         Caption         =   "refnum"
      EndProperty
      BeginProperty Field2 
         Precision       =   0
         Size            =   6
         Scale           =   0
         Type            =   129
         Name            =   "cuscde"
         Caption         =   "cuscde"
      EndProperty
      BeginProperty Field3 
         Precision       =   0
         Size            =   40
         Scale           =   0
         Type            =   200
         Name            =   "cusnam"
         Caption         =   "cusnam"
      EndProperty
      BeginProperty Field4 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ftramt"
         Caption         =   "ftramt"
      EndProperty
      BeginProperty Field5 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "ftrfee"
         Caption         =   "ftrfee"
      EndProperty
      BeginProperty Field6 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "ackno"
         Caption         =   "ackno"
      EndProperty
      BeginProperty Field7 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "cshamt"
         Caption         =   "cshamt"
      EndProperty
      BeginProperty Field8 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "adramt"
         Caption         =   "adramt"
      EndProperty
      BeginProperty Field9 
         Precision       =   8
         Size            =   19
         Scale           =   0
         Type            =   131
         Name            =   "adrnum"
         Caption         =   "adrnum"
      EndProperty
      BeginProperty Field10 
         Precision       =   9
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chgamt"
         Caption         =   "chgamt"
      EndProperty
      BeginProperty Field11 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno1"
         Caption         =   "chkno1"
      EndProperty
      BeginProperty Field12 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno2"
         Caption         =   "chkno2"
      EndProperty
      BeginProperty Field13 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno3"
         Caption         =   "chkno3"
      EndProperty
      BeginProperty Field14 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno4"
         Caption         =   "chkno4"
      EndProperty
      BeginProperty Field15 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkno5"
         Caption         =   "chkno5"
      EndProperty
      BeginProperty Field16 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt1"
         Caption         =   "chkamt1"
      EndProperty
      BeginProperty Field17 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt2"
         Caption         =   "chkamt2"
      EndProperty
      BeginProperty Field18 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt3"
         Caption         =   "chkamt3"
      EndProperty
      BeginProperty Field19 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt4"
         Caption         =   "chkamt4"
      EndProperty
      BeginProperty Field20 
         Precision       =   10
         Size            =   19
         Scale           =   2
         Type            =   131
         Name            =   "chkamt5"
         Caption         =   "chkamt5"
      EndProperty
      BeginProperty Field21 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk1"
         Caption         =   "chkbnk1"
      EndProperty
      BeginProperty Field22 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk2"
         Caption         =   "chkbnk2"
      EndProperty
      BeginProperty Field23 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk3"
         Caption         =   "chkbnk3"
      EndProperty
      BeginProperty Field24 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk4"
         Caption         =   "chkbnk4"
      EndProperty
      BeginProperty Field25 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   200
         Name            =   "chkbnk5"
         Caption         =   "chkbnk5"
      EndProperty
      BeginProperty Field26 
         Precision       =   0
         Size            =   3
         Scale           =   0
         Type            =   129
         Name            =   "status"
         Caption         =   "status"
      EndProperty
      BeginProperty Field27 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "rectag"
         Caption         =   "rectag"
      EndProperty
      BeginProperty Field28 
         Precision       =   0
         Size            =   10
         Scale           =   0
         Type            =   129
         Name            =   "userid"
         Caption         =   "userid"
      EndProperty
      BeginProperty Field29 
         Precision       =   23
         Size            =   16
         Scale           =   3
         Type            =   135
         Name            =   "sysdttm"
         Caption         =   "sysdttm"
      EndProperty
      BeginProperty Field30 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "updcde"
         Caption         =   "updcde"
      EndProperty
      BeginProperty Field31 
         Precision       =   0
         Size            =   1
         Scale           =   0
         Type            =   129
         Name            =   "ccrtyp"
         Caption         =   "ccrtyp"
      EndProperty
      NumGroups       =   0
      ParamCount      =   2
      BeginProperty P1 
         RealName        =   "Param1"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      BeginProperty P2 
         RealName        =   "Param2"
         Direction       =   1
         Precision       =   23
         Scale           =   3
         Size            =   16
         DataType        =   135
         HostType        =   7
         Required        =   -1  'True
      EndProperty
      RelationCount   =   0
      AggregateCount  =   0
   EndProperty
End
Attribute VB_Name = "zdeCCRRpt"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
